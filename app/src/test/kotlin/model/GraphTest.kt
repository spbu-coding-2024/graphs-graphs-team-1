...
class GraphTest {
    ...
    @MethodSource("generateVertices")
    fun `test edge deletion`(
        amount: Int,
        vertices: Array<Vertex<Int, Int>?>,
        kClass: KClass<Graph<Int, Int>>,
    ) {
        var graph = kClass.primaryConstructor?.call() ?: return
    ...
    fun `degree tests`(
        graph: Graph<Int, Int>,
        fromVertex: HashMap<Vertex<Int, Int>, Vector<Vertex<Int, Int>>>,
    ...
    fun `key value search`(
        graph: Graph<Int, Int>,
        keys: Array<Vector<Vertex<Int, Int>>>,
    ) {
    ...
    @MethodSource("generateVertices")
    fun `edges search functions test`(
        amount: Int,
        array: Array<Vertex<Int, Int>?>,
        kClass: KClass<Graph<Int, Int>>,
    ) {
        ...
        for (i in 0..1000) {
            ...
            var to = array.random()
            assertEquals(
                result.map { it.first === from && it.second === to }.contains(true),

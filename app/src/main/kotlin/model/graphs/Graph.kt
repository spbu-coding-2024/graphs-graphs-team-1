...
interface Graph<K, V> {
    ...
    fun containsEdge(
        from: Vertex<K, V>,
        to: Vertex<K, V>,
    ): Boolean = edges[from]?.map { it.link.second === to }?.contains(true) == true
    ...
    fun containsVertexWithKey(key: K): List<Vertex<K, V>> = vertices.filter { it.key == key }
    ...
    fun getOutDegreeOfVertex(vertex: Vertex<K, V>): Int = edges[vertex]?.size ?: 0
